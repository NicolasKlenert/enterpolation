...
use enterpolation::{Generator, DiscreteGenerator, bspline::BSpline};
...
fn main(){
    ...
    let spline = BSpline::builder()
        ...
        .equidistant()
        .degree(3).expect("As we have nearly infinite elements, we can have the degree as high as we want.")
        // We want each knot to lie on an integer.
        .distance(0.0, 1.0)
        .constant::<4>().expect("Degree of 3 needs space of 4")
        .build();
